...
void asio_server::run(port pt)
{
    ...
    boost::thread* new_thread = new boost::thread( boost::bind( &boost::asio::io_service::run, &io_service_) );
    io_threads_.push_back(new_thread);
}
